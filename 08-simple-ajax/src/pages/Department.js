...
const Department = () => {
    ...
    const onButtonClick = ReactCallback((e) => {
        ...
        setTimeout(() => {
            (async () => {
                ...
                try {
                    ...
                    //응답결과는 한 건의 JSON객체 --> {id: ?, dname: ?, loc: ?}
                    json = response.data;
                } catch (err) {
                    console.error(err);
    ...
    React.useEffect(() => {
        if(dropId>-1){
            /*
                ...
                setDepartment(department.filter((v, i) => v.id !== dripId));
    ...
    const onUpdateClick = React.useCallback(e => {
        e.preventDefault();
        //수정 할 항목에 대한 인덱스 번호를 상태값으로 설정한다.
        setUpdateId(parseInt(e.currentTarget.dataset.id));
    }, []);

    /* 수정사항 저장버튼이 클릭되었을 때 호출될 이벤트 핸들러 */
    //성능 최적화를 위해 useCallback()적용함
        ...
        (async() => {
            ...
            if(json!==null){
                ...
                setDepartment(department => {
                    //원본 상태값과 비교하여 수정된 ㅏㅎㅇ목의 배열인덱스를 찾는다.
                    const find = department.findIndex((v, i)=> v.id === json.id);
                    //원본 상태값의 해당 인덱스번호(find)위치부터 1개를 ajax로 반환받은 수정결과 데이터로 교체한다.
                            ...
                            return (
                                <tr key={item.id}>
                                    ...
                                    {updateId === item.id ? (
                                        ...
                                        <td><input type="hidden" name="loc" defaultValue={item.loc} /></td>
                                        <td colSpan="2">
